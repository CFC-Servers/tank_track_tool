...
function PANEL:AddNode( strName )
    ...
    local pNode = vgui.Create( "tanktracktoolEditor_Node", self )
...
function PANEL:Setup( editData )
    ...
    local control = "tanktracktoolEditor_" .. rowType

    if not vgui.GetControlTable( control ) then
        control = "tanktracktoolEditor_" .. ( tanktracktool.editor.types[string.lower( rowType )] or "Generic" )
    end

    if vgui.GetControlTable( control ) then
        self.Inner = self.Container:Add( control )
    else
        print( "tanktracktoolEditor: Failed to create panel (" .. control .. ")" )
    end
    if not IsValid( self.Inner ) then self.Inner = self.Container:Add( "tanktracktoolEditor_Generic" ) end
